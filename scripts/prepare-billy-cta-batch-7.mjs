...
function card(spec,slot,index){
 ...
 const aria=spec.language==='es'?`Billy Cooper Law: ayuda con ${spec.topic} en ${spec.location}. ${callLabel}.`:`Billy Cooper Law: ${spec.topic} help in ${spec.location}. ${callLabel}.`;
...
for(const spec of specs.filter(s=>s.featuredMedia && (!process.argv[2] || s.id===Number(process.argv[2])))){
 ...
 assert.equal(before.status.trim(),'Published');assert.equal(publicBefore.status,'publish');
