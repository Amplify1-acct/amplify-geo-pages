// Read-only saved-content and live-HTML verification for CTA rollout batch seven.
import fs from 'node:fs';
import assert from 'node:assert/strict';
import {parseDocument} from 'htmlparser2';
import {findAll,textContent,removeElement} from 'domutils';
const root='/Users/matthewsalvato/Documents/New project/wordpress-backups/billy-cta-batch-7-2026-09-04';
const ids=process.argv.slice(2).map(Number);
const selected=ids.length?ids:[5611,5126,3996,5136,5139,4582];
...
for(const id of selected){
 ...
 assert.equal(p.featured_media,prepared.featuredMedia,`${id}: wrong featured image`);
 ...
 const ordinary=await fetch(p.link);assert.equal(ordinary.status,200);const ordinaryDom=parseDocument(await ordinary.text(),{decodeEntities:true});assert.equal(find(ordinaryDom,e=>hasClass(e,'amplify-billy-batch-cta')).length,3,`${id}: stale canonical cache`);
 console.log(JSON.stringify({id,url:p.link,cards:3,language:prepared.language,localizedActionsVerified:true,approvedDesign:true,originalContentAndLinksUnchanged:true,metadataUnchanged:true,featuredMedia:p.featured_media}));
}
